requirejs(['ext_editor_io2', 'jquery_190', 'raphael_210'],
    function (extIO, $) {
        function overlappingDisksVisualization(tgt_node, data) {
            ...
            input.forEach((inp)=>{
                const [x, y, r] = inp.values
                const cx = (x-min_width)*unit
                const cy = (max_height-y)*unit
                paper.circle(cx, cy, r*unit).attr(attr.circle)
                // center
                paper.circle(cx, cy, .5).attr(attr.circle)
            })
